          ...
          <div className="flex flex-wrap gap-2">
            {isCategoriesLoading ? (
              ...
              Array.from({ length: 4 }).map((_, i) => (
                <div key={`cat-skeleton-${i}`} className="h-8 w-28 bg-gray-200 rounded-lg animate-pulse" />
              ))
            ) : (
              categories.map((cat) => {
                const isActive = cat._id === selectedCategory;
          ...
          <div className="flex flex-wrap gap-2">
            {isToolsLoading ? (
              ...
              Array.from({ length: 5 }).map((_, i) => (
                <div key={`tool-skeleton-${i}`} className="h-8 w-24 bg-gray-200 rounded-lg animate-pulse" />
              ))
            ) : tools.length > 0 ? (
              tools.map((tool) => {
                const isSelected = selectedTools.includes(tool.toolId);
