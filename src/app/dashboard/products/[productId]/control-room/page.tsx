...
export default function ControlRoomPage() {
  ...
  // Fetch Category List (Added isLoading mapping here)
  ...
  // Fetch Tools List ( Added isLoading mapping here)
  const { data: tools = [], isLoading: isToolsLoading } = useQuery<Tool[]>({
  ...
  const selectedVisibleCount = resources.filter((res) =>
    selectedResourceIds.includes(res._id),
  ).length;
  const isEveryRowChecked =
    totalVisibleItemsCount > 0 &&
    selectedVisibleCount === totalVisibleItemsCount;
      ...
      <div className="mx-auto max-w-7xl bg-white p-6">
              ...
              : categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 cursor-pointer rounded-lg border text-sm transition-colors ${
                        isActive
                    ...
                    >
                      {cat.image && (
                        <img
                          ...
                          style={{
                            filter: isActive
                              ? "invert(32%) sepia(90%) md(0deg) saturate(1814%) hue-rotate(212deg) brightness(97%) contrast(93%)"
                              : "none",
                          }}
                        />
                      )}
              ...
              tools.map((tool) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer rounded-lg border text-sm transition-colors ${
                      isSelected
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                router.push(
                  `/dashboard/products/${productId}/control-room/add-bulk-resource`,
                )
              }
              className="px-4 py-2 border border-gray-200 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition-colors cursor-pointer"
            >
              Add Bulk
            </button>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 mb-4">
          ...
          <FilterSelect
            ...
            options={[
              { label: "Filter by resource type", value: "All" },
        ...
        <div className="overflow-x-auto border border-gray-100 rounded-xl">
          <table className="w-full min-w-200 border-collapse text-left text-sm text-gray-700">
            ...
            <tbody className="divide-y divide-gray-100">
              {isResourcesLoading ? (
                /* HIGH-FIDELITY TABLE SKELETON ROW GENERATOR */
                Array.from({ length: PAGE_SIZE }).map((_, index) => (
                  <tr key={`table-skeleton-${index}`} className="animate-pulse">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-5 text-sm text-gray-500">
          <div>
            Showing {Math.min(resources.length, PAGE_SIZE)} out of{" "}
